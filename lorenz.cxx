...
void f (const double* const y, double* const f) {
	f[0] = a * (y[1]-y[0]);
	f[1] = y[0] * (b-y[2]) - y[1];
	f[2] = y[0]*y[1] - c*y[2];
}
...
void fill_k (const double* const y, const double dt) {
	...
	// variable to store values for evaluating different k
	double temp[3];
...
int main (int argcount, char** argvector) {
	...
	if (argcount != 2) {
		...
		cout << "usage: " << argvector[0] << " dt" << endl;
		return -1;
	}
